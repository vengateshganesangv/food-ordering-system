...
export class PaymentDomainServiceImpl implements PaymentDomainService {
  validateAndInitiatePayment(
    ...
    if (failureMessages.length === 0) {
      console.log(`Payment is initiated for order id: ${payment.orderId.getValue()}`);
      payment.updateStatus(PaymentStatus.COMPLETED);
      return new PaymentCompletedEvent(payment, new Date());
    } else {
      console.log(`Payment initiation is failed for order id: ${payment.orderId.getValue()}`);
  ...
  validateAndCancelPayment(
    ...
    if (failureMessages.length === 0) {
      console.log(`Payment is cancelled for order id: ${payment.orderId.getValue()}`);
      payment.updateStatus(PaymentStatus.CANCELLED);
      return new PaymentCancelledEvent(payment, new Date());
    } else {
      console.log(`Payment cancellation is failed for order id: ${payment.orderId.getValue()}`);
      payment.updateStatus(PaymentStatus.FAILED);
      return new PaymentFailedEvent(payment, new Date(), failureMessages);
    }
  }
  ...
  private validateCreditEntry(payment: Payment, creditEntry: CreditEntry, failureMessages: string[]): void {
    if (payment.price.isGreaterThan(creditEntry.totalCreditAmount)) {
      console.error(`Customer with id: ${payment.customerId.getValue()} doesn't have enough credit for payment!`);
      failureMessages.push(
        `Customer with id=${payment.customerId.getValue()} doesn't have enough credit for payment!`
      );
    }
  }
  ...
  private validateCreditHistory(
    ...
    if (totalDebitHistory.isGreaterThan(totalCreditHistory)) {
      console.error(
        `Customer with id: ${creditEntry.customerId.getValue()} doesn't have enough credit according to credit history`
      );
      failureMessages.push(
        `Customer with id=${creditEntry.customerId.getValue()} doesn't have enough credit according to credit history!`
      );
    ...
    if (!creditEntry.totalCreditAmount.equals(totalCreditHistory.subtract(totalDebitHistory))) {
      console.error(
        `Credit history total is not equal to current credit for customer id: ${creditEntry.customerId.getValue()}!`
      );
      failureMessages.push(
        `Credit history total is not equal to current credit for customer id: ${creditEntry.customerId.getValue()}!`
      );
  ...
  private getTotalHistoryAmount(creditHistories: CreditHistory[], transactionType: TransactionType): Money {
    return creditHistories
      ...
      .map((creditHistory) => creditHistory.amount)
