...
const logger = new Logger('RestaurantServiceApplication');
...
async function bootstrap() {
  try {
    ...
    app.get('/health', (req, res) => {
      res.status(200).json({ status: 'UP', service: 'restaurant-service' });
    });
    ...
    const schedulerInterval = Math.floor(config.restaurantService.outboxSchedulerFixedRate / 1000);
